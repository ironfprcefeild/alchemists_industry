...
public class ModCreativeModeTabs {
    private static final DeferredRegister<CreativeModeTab> REGISTER =
            DeferredRegister.create(Registries.CREATIVE_MODE_TAB, Alchemind.MODID);

    public static final RegistryObject<CreativeModeTab> BASE_CREATIVE_TAB = REGISTER.register("base",
            () -> CreativeModeTab.builder()
                    .title(Components.translatable("itemGroup.alchemindtab"))
                    .withTabsBefore(CreativeModeTabs.SPAWN_EGGS)
                    .icon(() -> ModItems.BLANK_SIGIL.asStack())
    ...
    private static class RegistrateDisplayItemsGenerator implements CreativeModeTab.DisplayItemsGenerator {
        ...
        public RegistrateDisplayItemsGenerator(boolean addItems) {
            ...
            this.tabFilter = BASE_CREATIVE_TAB;
        }
        ...
        private static Predicate<Item> makeExclusionPredicate() {
            ...
            for (ItemProviderEntry<?> entry : simpleExclusions) {
                exclusions.add(entry.asStack().getItem());
            }
        ...
        private static List<RegistrateDisplayItemsGenerator.ItemOrdering> makeOrderings() {
            ...
            Map<ItemProviderEntry<?>, ItemProviderEntry<?>> simpleBeforeOrderings = Map.of(
                    ModItems.BLANK_SIGIL, ModItems.AQUA_SIGIL,
                    ModItems.HARD_GALAXIUM, ModItems.GALAXIUM_SHARD,
                    ModItems.GLAMERITE, ModItems.GLAMERITE_POWDER,
                    ModItems.REACTIVITE, ModItems.REACTIVITE_POWDER,
                    ModItems.GLAMERITE_POWDER,ModItems.GLAMERITE_ALLOY,
                    ModItems.REACTIVITE_POWDER, ModItems.REACTIVITE_ALLOY,
                    ModItems.PYRITE, ModItems.PYRITE_POWDER
            );

            Map<ItemProviderEntry<?>, ItemProviderEntry<?>> simpleAfterOrderings = Map.of(
                    ModItems.CHARGED_STONE, ModItems.SUPER_CHARGED_STONE
            );

            simpleBeforeOrderings.forEach((entry, otherEntry) -> {
                orderings.add(RegistrateDisplayItemsGenerator.ItemOrdering.before(entry.asStack().getItem(), otherEntry.asStack().getItem()));
            });

            simpleAfterOrderings.forEach((entry, otherEntry) -> {
                orderings.add(RegistrateDisplayItemsGenerator.ItemOrdering.after(entry.asStack().getItem(), otherEntry.asStack().getItem()));
            });
        ...
        private static Function<Item, ItemStack> makeStackFunc() {
            ...
            simpleFactories.forEach((entry, factory) -> {
                factories.put(entry.asStack().getItem(), factory);
            });
        ...
        private static Function<Item, CreativeModeTab.TabVisibility> makeVisibilityFunc() {
            ...
            simpleVisibilities.forEach((entry, factory) -> {
                visibilities.put(entry.asStack().getItem(), factory);
            });

            for (BlockEntry<ValveHandleBlock> entry : AllBlocks.DYED_VALVE_HANDLES) {
                visibilities.put(entry.asStack().getItem(), CreativeModeTab.TabVisibility.SEARCH_TAB_ONLY);
            }
